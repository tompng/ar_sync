...
export default class ArSyncStore {
  ...
  _slicePatch(patchData, request) {
    ...
    if (patchData.id) obj.id = patchData.id
    return obj
  }
  ...
  _update(patch: { action: string; path: (string | number)[]; ordering; data }, updator, events) {
    const { action, path } = patch
    const patchData = patch.data
    let request = this.request
    let data = this.data
    const trace = (i: number, actualPath: (string | number)[], accessKeys: (string | number)[], query, data) => {
      ...
      if (typeof(nameOrId) === 'number') {
        ...
        for (const key in query) {
          ...
          if (field === nameOrId) matchedKeys.push(key)
        }
    ...
    const apply = (accessKeys: (string | number)[], actualPath: (string | number)[], query, column: string | null, idx: number | null, target) => {
      if (action === 'create') {
